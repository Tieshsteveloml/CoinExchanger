let reviews = [];
let vol_trigger = true;
let vol_function = false;
...
var myObject = new Vue({
    ...
    data: {
        ...
        split_out_tx_list: [],
        time_interval: 5000,
        time_interval_last_orders: 22000,
        page_no_out_tx: 0,
        max_out_tx_count: 1,
        refunded_tx: "",
        dst_tx: "",
    ...
    mounted() {
        ...
        //this.get_last_completed_orders();
        this.interval_last_orders();
    ...
    methods: {
        submit_support () {
            let valid = true;
            if (this.support_name === '')
            {
                this.invalid_support_name = 'PLEASE ENTER YOUR NAME!';
                setTimeout(function () {
                        myObject.invalid_support_name = "";
                    }, 3000);
                valid = false;
            }
            if (this.support_email === '' || validateEmail(this.support_email) === false)
            {
                this.invalid_support_email = 'PLEASE ENTER VALIDATE EMAIL!';
                setTimeout(function () {
                        myObject.invalid_support_email = "";
                    }, 3000);
                valid = false;
            }
            let spas = document.getElementsByClassName('custom-select-trigger');
            if (spas.length > 0)
            {
                this.support_subject = spas[0].innerHTML;
            }
            if (this.support_subject === "" || this.support_subject === "Select subject" || this.support_subject === "PLEASE SELECT SUBJECT!")
            {
                if (spas.length > 0)
                {
                    spas[0].innerHTML = 'PLEASE SELECT SUBJECT!';
                    spas[0].className += ' span-invalid-color';
                }
                this.invalid_support_subject = 'PLEASE SELECT SUBJECT!';
                setTimeout(function () {
                        myObject.invalid_support_subject = "";
                        spas[0].innerHTML = 'Select subject';
                        spas[0].className = 'custom-select-trigger';
                    }, 3000);
                valid = false;
            }
            if (this.support_description === '')
            {
                this.invalid_support_description = 'PLEASE DESCRIBE YOUR SUBJECT!';
                setTimeout(function () {
                        myObject.invalid_support_description = "";
                    }, 3000);
                valid = false;
            }
            if (!valid) return;
            let param = {
                username: this.support_name,
                user_email: this.support_email,
                subject: this.support_subject,
                deposit_address: this.support_deposit_address,
                description: this.support_description
            };
            $.ajax({
                url: "submit_support",
                type: "POST",
        ...
        make_last_orders_div (rand) {
            if (this.last_orders === null || this.last_orders.length === 0) return;
            let temps = [];
            for (let i = 0; i < this.last_orders.length; i++)
            {
                let one = '<div id="id-' + Math.random().toString() + '" ';
                if (this.last_orders[i].source_coin === "BTC")
                {
                    if (this.is_dark_mode)
                    {
                        one += 'class="footer-last-order-blue dark-footer-last-order">';
                        one += '<div class="custom_row last-order-volume"><div class="custom_col order-volume-bar dark-blue-order-volume-bar">';
                    }
                    else
                    {
                        one += 'class="footer-last-order-blue white-footer-last-order">';
                        one += '<div class="custom_row last-order-volume"><div class="custom_col order-volume-bar white-blue-order-volume-bar">';
                    }
                }
                else
                {
                    if (this.is_dark_mode)
                    {
                        one += 'class="footer-last-order-orange dark-footer-last-order">';
                        one += '<div class="custom_row last-order-volume"><div class="custom_col order-volume-bar dark-orange-order-volume-bar">';
                    }
                    else
                    {
                        one += 'class="footer-last-order-orange white-footer-last-order">';
                        one += '<div class="custom_row last-order-volume"><div class="custom_col order-volume-bar white-orange-order-volume-bar">';
                    }
                }
                ...
                {
                    if (this.last_orders[i].source_coin === "BTC")
                        bar_width = parseInt(Math.round(this.last_orders[i].received / this.day_volume * 100));
                    else
                        bar_width = parseInt(Math.round(this.last_orders[i].dst_amt / this.day_volume * 100));
                }
                if (this.last_orders[i].source_coin === "BTC")
                {
                    one += '<div id="volume-bar" class="blue-order-volume-active-bar" style="width: ' + bar_width.toString() + '%"></div></div>';
                }
                else
                {
                    one += '<div id="volume-bar" class="orange-order-volume-active-bar" style="width: ' + bar_width.toString() + '%"></div></div>';
                }
                /*if (this.last_orders[i].source_coin === "BTC")
                    one += '<div class="orange-order-volume-active-bar" style="width: ' + bar_width.toString() + '%"></div></div>';
                else
                    one += '<div class="blue-order-volume-active-bar" style="width: ' + bar_width.toString() + '%"></div></div>';*/
                if (this.is_dark_mode)
                {
                    one += '<div class="custom_col order-volume-description dark-order-volume-description">';
                    one += '<p id="nbr" align="right" style="margin-bottom: 0;">';
                }
                else {
                    one += '<div class="custom_col order-volume-description white-order-volume-description">';
                    one += '<p id="nbr" align="right" style="margin-bottom: 0;">';
                }
                one += bar_width.toString() + '% VOL</p>';
                one += '</div></div>';
                one += '<div class="custom_row last-order-result"><div class="custom_col order-result-icon">';
                if (this.last_orders[i].source_coin === "BTC")
                    one += '<img src="' + get_icon_path(this.last_orders[i].dest_coin) + '" alt="Image" width="30" height="30" style="display: block;"/></div>';
                else
                    one += '<img src="' + get_icon_path(this.last_orders[i].source_coin) + '" alt="Image" width="30" height="30" style="display: block;"/></div>';

                if (this.is_dark_mode)
                    one += '<div class="custom_col order-result-serve"><div class="custom_row order-result-amount dark-order-result-amount">';
                else
                    one += '<div class="custom_col order-result-serve"><div class="custom_row order-result-amount white-order-result-amount">';
                if (this.last_orders[i].source_coin === "BTC")
                {
                    let amount = this.last_orders[i].received.toFixed(3);
                    if (amount <= 0) amount = 0.001;
                    one += '<p class="footer-p-style">' + amount.toString() + ' BTC</p></div>';
                }
                else
                {
                    let amount = this.last_orders[i].dst_amt.toFixed(3);
                    if (amount <= 0) amount = 0.001;
                    one += '<p class="footer-p-style">' + amount.toString() + ' BTC</p></div>';
                }
                if (this.is_dark_mode)
                    one += '<div class="custom_row order-result-processed dark-order-result-processed"><p class="footer-p-style">';
                else
                    one += '<div class="custom_row order-result-processed white-order-result-processed"><p class="footer-p-style">';
                let times = 'instant';
                if (this.last_orders[i].out_tx_list.length > 0)
                {
                    let processed = new Date(this.last_orders[i].end_date) - new Date(this.last_orders[i].out_tx_list[0].time);
                    processed = processed / 1000 - this.last_orders[i].out_tx_list[0].took_time;
                    processed /= 60;
                    ...
                    {
                        times = 'for ' + parseInt(processed).toString() + ' mins';
                    }
                }
                one += 'served ' + times + '</p></div></div>';
                if (this.is_dark_mode)
                    one += '<div class="custom_col order-result-time dark-order-result-time">';
                else
                    one += '<div class="custom_col order-result-time white-order-result-time">';
                let before = (new Date(this.server_time) - new Date(this.last_orders[i].end_date)) / 1000 / 60;
                let happens = '1 min ago';
                if (before < 60)
                {
                    if (parseInt(before) <= 0)
                        before = 1;
                    if (parseInt(before) === 1)
                        happens = parseInt(before).toString() + ' min ago';
                    else
                        happens = parseInt(before).toString() + ' mins ago';
                }
                else if (before < 1440)
                {
                    before /= 60;
                    if (parseInt(before) <= 0)
                        before = 1;
                    if (parseInt(before) === 1)
                        happens = parseInt(before).toString() + ' hour ago';
                    else
                        happens = parseInt(before).toString() + ' hours ago';
                }
                else if (before < 43200)
                {
                    before = before / 60 / 24;
                    if (parseInt(before) <= 0)
                        before = 1;
                    if (parseInt(before) === 1)
                        happens = parseInt(before).toString() + ' day ago';
                    else
                        happens = parseInt(before).toString() + ' days ago';
                }
                else if (before < 525600)
                {
                    before = before / 60 / 24 / 30;
                    if (parseInt(before) <= 0)
                        before = 1;
                    if (parseInt(before) === 1)
                        happens = parseInt(before).toString() + ' month ago';
                    else
                        happens = parseInt(before).toString() + ' months ago';
                }
                else
                {
                    before = before / 60 / 24 / 365;
                    if (parseInt(before) <= 0)
                        before = 1;
                    if (parseInt(before) === 1)
                        happens = parseInt(before).toString() + ' year ago';
                    else
                        happens = parseInt(before).toString() + ' years ago';
                }
                one += '<p class="footer-p-style">' + happens + '</p></div></div>';
                one += '</div>';
                temps.push(one);
            }
            this.last_orders_div = temps;
            if (this.last_orders_div.length === 0)
            {
                if (!rand) return;
                this.last_order_index = -1;
                this.last_order_one = '';
            }
            else if (this.last_orders_div.length === 1)
            {
                this.last_order_one = this.last_orders_div[0];
                this.last_order_index = 0;
            }
            else
            {
                let last_index;
                if (rand || this.last_order_index >= this.last_orders_div.length)
                    last_index = Math.floor(Math.random() * this.last_orders_div.length);
                else
                {
                    last_index = this.last_order_index;
                }
                this.last_order_one = this.last_orders_div[last_index];
                this.last_order_index = last_index;
            }
        },
        timerEnd() {
        ...
        get_last_completed_orders() {
            axios.get(`${this.baseUrl}/get_last_completed_orders`)
            .then((response) =>
            {
                this.last_orders = response.data;
                this.make_last_orders_div(true);
            });
        },
        get_24h_volume_and_time() {
            axios.get(`${this.baseUrl}/get_24h_volume_and_time`)
            .then((response) => {
                this.day_volume = response.data.volume;
                this.day_volume_int = parseInt(this.day_volume);
                this.average_time = parseInt(response.data.time);
                this.get_last_completed_orders();
            });
        },
        get_server_time() {
            axios.get(`${this.baseUrl}/get_server_time`)
            .then((response) => {
        ...
        calc_ex_price_max_btc() {
            for (let i = 0; i < price_list.length; i++) {
                if (price_list[i].name === this.order_type) {
                    ...
                    this.server_time = price_list[i].server_time;
                    if (this.source_cur === "BTC") {
        ...
        interval_last_orders: function () {
            setInterval(() => {
                this.get_server_time();
                if (new Date().getHours() !== this.current_hours)
                {
                    this.get_24h_volume_and_time();
                    this.current_hours = new Date().getHours();
                }
                else
                    this.make_last_orders_div(true);
                vol_trigger = true;
            }, this.time_interval_last_orders);
        },
        ...
        onclick_switch_theme() {
            ...
            this.make_last_orders_div(false);
            sleep(3000).then(() => {
...
function incEltNbrwithoutEnd(id) {
...
function incEltNbr(id, endNum) {
  elt = document.getElementById(id);
  if (!elt)
      return;
  incNbrRec(0, endNum, elt);
  vol_trigger = false;
}
...
function incNbrRec(i, endNbr, elt) {
  if (i <= endNbr) {
    ...
    }, speed + deceleration * i);
  }
}

function incBarRec(i, endNbr, elt) {
  if (i <= endNbr) {
    elt.style.width = i + "%";
    setTimeout(function() {//Delay a bit before calling the function again.
      incBarRec(i + 1, endNbr, elt);
    }, speed + deceleration * i);
  }
}
...
function incBar(id) {
    ...
    let percent = 200 / elt.clientWidth * 100;
    incBarRec(0, percent, elt);
    return true;
}

function incNbr(){
    if (vol_function) return;
    vol_function = true;
    elt = document.getElementById("volume-bar");
    ...
    setTimeout(function() {//Delay a bit before calling the function again.
        let res = incBar("volume-bar");
        if (res)
        {
            if (endNbr >= 0)
            {
                incEltNbr("nbr", endNbr);
            }
            else
            {
                incEltNbrwithoutEnd("nbr");
            }
        }
        vol_function = false;
    }, 500);
